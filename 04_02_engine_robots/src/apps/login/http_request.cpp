...
bool HttpRequest::ProcessSend() {
    ...
    curl_easy_setopt(_pCurl, CURLOPT_WRITEFUNCTION, WriteFunction); // 设置写入数据的回调函数
    curl_easy_setopt(_pCurl, CURLOPT_WRITEDATA, static_cast<void *>(&_responseBuffer)); // 设置写入数据的缓冲区
...
void HttpRequest::ProcessTimeout() const {
  ...
  checkProto.set_return_code(
      Proto::AccountCheckRs::ARC_TIMEOUT); // 设置超时返回码
  auto pCheckPacket = new Packet(Proto::MsgId::MI_AccountCheckToHttpRs, 0);
  pCheckPacket->SerializeToBuffer(checkProto); // 序列化数据到缓冲区
  DisPatchPacket(pCheckPacket);                // 分发数据包
}
...
void HttpRequest::ProcessMsg() {
    int msgs_left;
    CURLMsg *msg = curl_multi_info_read(_pMultiHandle, &msgs_left); // 读取多句柄信息
    ...
    if (msg->msg == CURLMSG_DONE) {
        ...
        if (msg->data.result != CURLE_OK) {
            std::cerr << "Curl request failed: " << curl_easy_strerror(msg->data.result) << std::endl;
            ProcessTimeout(); // 处理超时情况
            return;
        }

        // 请求成功，输出响应内容
        std::cout << "Curl request succeeded. Response: " << _responseBuffer << std::endl;
        ...
        if (ok && errs.empty()) {
            ...
            std::cerr << "JSON parse failed. Errors: " << errs << " Response: " << _responseBuffer << std::endl;
        }
    } else {
        std::cerr << "ProcessMsg: Unhandled CURLMsg type: " << msg->msg << std::endl;
    }
}
