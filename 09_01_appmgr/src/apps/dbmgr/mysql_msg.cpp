...
void MysqlConnector::InitMessageComponent() {
  ...
  AddComponent<MessageComponent>(pMsgCallBack); // 关联回调处理程序
...
void MysqlConnector::HandleCreatePlayer(Packet *pPacket) {
  ...
  protoRs.set_return_code(
      Proto::CreatePlayerReturnCode::CPR_Unkonwn); // 设置默认返回码
...
bool MysqlConnector::OnSavePlayer(DatabaseStmt *stmtSave,
  ...
  AddParamBlob(stmtSave, (void *)baseStr.c_str(),
               (int)baseStr.size()); // 绑定玩家基础信息

  std::string miscStr;
  protoPlayer.misc().SerializeToString(&miscStr);
  AddParamBlob(stmtSave, (void *)miscStr.c_str(),
               (int)miscStr.size()); // 绑定玩家其他信息
