...
void ConsoleCmdTrace::RegisterHandler() {
  OnRegisterHandler("-connect",
                    BindFunP1(this, &ConsoleCmdTrace::HandleConnect));
  ...
  OnRegisterHandler("-account",
                    BindFunP1(this, &ConsoleCmdTrace::HandleAccount));

  OnRegisterHandler("-clean", BindFunP1(this, &ConsoleCmdTrace::HandleClean));
}
void ConsoleCmdTrace::HandleHelp() {
  std::cout << "\t-connect socket.  \t\tshow the information of socket."
            << std::endl;
  std::cout << "\t-packet socket.  \t\tall trace of packet related socket."
            << std::endl;
  std::cout << "\t-player socket.  \t\tall trace of player related socket."
            << std::endl;
...
void ConsoleCmdTrace::HandlePacket(std::vector<std::string> &params) {
  if (!CheckParamCnt(params, 1))
    return;
  const int socket = std::atoi(params[0].c_str());
  auto pTraceComponet = ComponentHelp::GetTraceComponent();
  pTraceComponet->Show(TraceType::Packet, socket);
}
void ConsoleCmdTrace::HandlePlayer(std::vector<std::string> &params) {
  ...
  const int socket = std::atoi(params[0].c_str());
  auto pTraceComponet = ComponentHelp::GetTraceComponent();
  pTraceComponet->Show(TraceType::Player, socket);
}
void ConsoleCmdTrace::HandleAccount(std::vector<std::string> &params) {
  ...
  auto pTraceComponet = ComponentHelp::GetTraceComponent();
  pTraceComponet->ShowAccount(account);
}
