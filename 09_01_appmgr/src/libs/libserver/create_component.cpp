...
IComponent *ComponentFactoryEx(EntitySystem *pEntitySystem,
...
template <size_t ICount> struct DynamicCall {
  template <typename... TArgs>
  static IComponent *
  Invoke(EntitySystem *pEntitySystem, const std::string classname,
         std::tuple<TArgs...> t1,
    ...
    if (param.type() == Proto::CreateComponentParam::Int) {
      ...
      return DynamicCall<ICount - 1>::Invoke(pEntitySystem, classname, t2,
                                             params);
    }
...
void CreateComponentC::Awake() {
  ...
  pMsgCallBack->RegisterFunction(
      Proto::MsgId::MI_CreateComponent,
      BindFunP1(this, &CreateComponentC::HandleCreateComponent));
  pMsgCallBack->RegisterFunction(
      Proto::MsgId::MI_RemoveComponent,
      BindFunP1(this, &CreateComponentC::HandleRemoveComponent));
}
...
void CreateComponentC::HandleCreateComponent(Packet *pPacket) const {
  Proto::CreateComponent proto =
      pPacket->ParseToProto<Proto::CreateComponent>();
  ...
  if (proto.params_size() >= 5) {
    std::cout << " !!!! CreateComponent failed. className:" << className.c_str()
              << " params size >= 5" << std::endl;
    return;
  }

  auto params = proto.params();
  const auto pObj =
      DynamicCall<5>::Invoke(GetSystemManager()->GetEntitySystem(), className,
