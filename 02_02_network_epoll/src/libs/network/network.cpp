...
#else
#define SetsockOptType cost char *
#endif
void Network::SetSocketOpt(SOCKET socket) {
  ...
  setsockopt(socket, SOL_SOCKET, SO_KEEPALIVE, (SetsockOptType)&keepAlive,
             optlen);
  if (getsockopt(socket, SOL_SOCKET, SO_KEEPALIVE, (SetsockOptType)&keepAlive,
                 &optlen) < 0) {
    std::cout << "getsockopt SO_KEEPALIVE failed." << std::endl;
  }

  setsockopt(socket, SOL_TCP, TCP_KEEPIDLE, (SetsockOptType)&keepIdle, optlen);
  if (getsockopt(socket, SOL_TCP, TCP_KEEPIDLE, (SetsockOptType)&keepIdle,
                 &optlen) < 0) {
    std::cout << "getsockopt TCP_KEEPIDLE failed." << std::endl;
  }
  setsockopt(socket, SOL_TCP, TCP_KEEPINTVL, (SetsockOptType)&keepInterval,
             optlen);
  setsockopt(socket, SOL_TCP, TCP_KEEPCNT, (SetsockOptType)&keepCount, optlen);
#endif
