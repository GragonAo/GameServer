...
bool ConnectObj::Recv() const {
  ...
  while (true) {
    ...
    if (dataSize > 0) {
      std::cout << "recv size:" << dataSize << std::endl;
      _recvBuffer->FillData(dataSize);
    } else if (dataSize == 0) {
      std::cout << "recv size:" << dataSize << " error:" << _sock_err()
                << std::endl;
      return false;
    } else {
      ...
      if (socketError == WSAEINTR || socketError == WSAEWOULDBLOCK)
        return true;
#endif
      std::cout << "recv size:" << dataSize << " error:" << socketError
                << std::endl;
      return false;
    }
  }
...
bool ConnectObj::Send() const {
  while (true) {
    ...
    if (size == -1) {
      const auto socketError = _sock_err();
      std::cout << "needSendSize: " << needSendSize << "error:" << socketError
                << std::endl;
      return false;
    }
  }
