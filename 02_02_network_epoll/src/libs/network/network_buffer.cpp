...
Packet *RecvNetworkBuffer::GetPacket() {
  ...
  const auto dataLength =
      totalSize - sizeof(PacketHead) - sizeof(TotalSizeType);
...
SendNetworkBuffer::SendNetworkBuffer(const unsigned int _size)
    : NetworkBuffer(_size) {}
...
void SendNetworkBuffer::AddPacket(Packet *pPacket) {
  ...
  TotalSizeType totalSize =
      dataLength + sizeof(PacketHead) + sizeof(TotalSizeType);
  while (GetEmptySize() < totalSize) {
    ReAllocBuffer();
  }

  // std::cout << "send buffer::Realloc. _bufferSize:" << _bufferSize <<
  // std::endl;
