...
#ifndef WIN32
#define SetsocketOptType void *
#else
#define SetsocketOptType char *
#endif
void Network::SetSocketOpt(SOCKET socket) {
  bool isReuseaddr = true;
  setsockopt(socket, SOL_SOCKET, SO_REUSEADDR, (SetsocketOptType)&isReuseaddr,
             sizeof(isReuseaddr));

  int netTimeout = 3000;
  setsockopt(socket, SOL_SOCKET, SO_SNDTIMEO, (SetsocketOptType)&netTimeout,
             sizeof(netTimeout));
  setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO, (SetsocketOptType)&netTimeout,
             sizeof(netTimeout));
#ifndef WIN32
  int keepAlive = 1;
  ...
  int keepInterval = 10;
  int keepCount = 5;
  setsockopt(socket, SOL_SOCKET, SO_KEEPALIVE, (SetsocketOptType)&keepAlive,
             optlen);
  if (getsockopt(socket, SOL_SOCKET, SO_KEEPALIVE, &keepAlive, &optlen)) {
    std::cout << "getsocket SO_KEEPALIVE failed." << std::endl;
  }
