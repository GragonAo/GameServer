...
void Account::RegisterMsgFunction() {
  ...
  pMsgCallBack->RegisterFunction(Proto::MsgId::MI_AccountCheckToHttpRs,
  ...
  pMsgCallBack->RegisterFunction(Proto::MsgId::L2DB_CreatePlayerRs,
  BindFunP1(this, &Account::HandleCreatePlayerRs));

  pMsgCallBack->RegisterFunction(Proto::MsgId::L2DB_QueryPlayerListRs, 
...
void Account::HandleNetworkDisconnect(Packet *pPacket) {
  // std::cout << "player offline. socket:" << pPacket->GetSocket() <<
  // std::endl;
...
void Account::HandleAccountCheck(Packet *pPacket) {
  ...
  if (pPlayer != nullptr) {
    ...
    IMessageSystem::DispatchPacket(Proto::MsgId::MI_NetworkDisconnectToNet,
                                   socket);
...
void Account::HandleAccountCheckToHttpRs(Packet *pPacket) {
  ...
  SendPacket(Proto::MsgId::C2L_AccountCheckRs, pPlayer->GetSocket(),
             protoResult);
...
void Account::HandleCreatePlayerRs(Packet *pPacket) {
  ...
  SendPacket(Proto::MsgId::C2L_CreatePlayerRs, pPacket->GetSocket(),
             createProto);
}